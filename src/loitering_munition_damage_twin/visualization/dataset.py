...
MUN_NAMES = {0: "Small", 1: "Med-LM", 2: "Med-RD", 3: "Heavy"}
...
def module_01_spatial(df: pd.DataFrame, out_dir: Path):
    ...
    ax3.plot(r_grid, pdf_shell, "k--", lw=2,
             label=fr"Shell volumetric $f(r)=3r^2/(R^3-r_0^3)$, $r_0={r_min:.0f},R={r_max:.0f}$")
    ax3.plot(r_grid, pdf_uniform, color="#888", lw=1.5, linestyle=":",
             label="Radially uniform reference")
    ax3.set_title("Radial Distance Density Check")
    ax3.set_xlabel("Radial Distance R (cm)")
...
def module_02_kinematic(df: pd.DataFrame, out_dir: Path):
    ...
    ax1.set_title("Velocity Norm Distribution")
    ...
    ax2.plot(np.cos(theta), np.sin(theta), "k--", lw=1, alpha=0.7,
             label="unit circle")
    ax2.set_title("Yaw Phase Manifold (cos, sin)")
    ax2.set_aspect("equal", adjustable="datalim")
...
def module_03_aero_ipw(df: pd.DataFrame, out_dir: Path):
    ...
    ax1.axvline(15, color="k", linestyle="--", label="15° soft tail start")
    ax1.axvline(30, color="k", linestyle="-", label="30° hard cutoff")
    ax1.set_title("Aerodynamic AoA Marginal Distribution")
    ax1.set_xlabel("Reconstructed AoA (deg)")
    ax1.legend()
    ...
    ax2.set_yscale("log")
    ax2.set_title("IPW Loss-Weight Response vs AoA  (log y)")
    ax2.set_xlabel("Aerodynamic AoA (deg)")
    ax2.set_ylabel("loss_weight  (log)")
...
def module_04_damage(df: pd.DataFrame, out_dir: Path):
    ...
    ax1.set_title("Munition ID Assignment Equality")
    ax1.set_xlabel("Munition Types (0..3)")

    ax2 = fig.add_subplot(132)
    sns.violinplot(x="munition_id", y="overall_score", hue="munition_id",
                   data=df, palette="Set2", inner="quartile",
                   legend=False, ax=ax2)
    ax2.set_title("Overall Damage Score Stratification")
    ax2.set_xlabel("Munition Types")
    ...
    ax3.set_title("Physics × Damage Spearman Correlation")
...
def module_05_feature_marginal(df: pd.DataFrame, integrity: dict, out_dir: Path):
    ...
    for spare_ax in axes[panel_idx + 1:]:
        spare_ax.axis("off")
    fig.suptitle("Feature Marginal Distributions (15 dim) + Integrity Audit",
                 fontsize=14, y=1.0)
    plt.tight_layout()
...
def module_06_feature_label_corr(df: pd.DataFrame, out_dir: Path):
    ...
    ax1.set_title("Spearman rho:  Feature x Ordinal Label")
    ...
    ax2.set_title("Pearson rho:  Feature x Feature  (collinearity)")
...
def module_07_label_balance(df: pd.DataFrame, out_dir: Path):
    ...
    for i, lab in enumerate(LABEL_COLUMNS):
        ...
        sns.heatmap(ct, annot=True, fmt="d", cmap="YlOrRd",
                    cbar=True, ax=ax)
        ax.set_title(f"Counts:  {lab}  per munition")
        ax.set_xlabel("class level")
        ax.set_ylabel("munition")
    ...
    for ax, (a_col, b_col) in zip(sub_axes, pairs):
        ...
        ax.set_title(f"Soft labels:  {a_col[0]}-task")
        ax.legend(fontsize=9)

    fig.suptitle("Label Balance (Hard + Soft Targets)", fontsize=14, y=1.0)
    plt.tight_layout()
...
def module_08_weight_split(df: pd.DataFrame, out_dir: Path,
    ...
    txt = (f"ESS = {ess:,.0f}  ({ess_ratio*100:.1f}% of N)\n"
           f"p99 = {p99:.1f}\n"
           f"p99.9 = {p999:.1f}\n"
           f"max = {w.max():.1f}")
    ...
    ax.set_title("loss_weight Tail & Effective Sample Size")
    ...
    ax.set_yscale("log")
    ax.set_title("loss_weight by (munition, K_level)  - IPW shape check")
    ...
    sns.heatmap(layer_ct, annot=True, fmt="d", cmap="Blues",
                cbar=True, ax=ax)
    ax.set_title("Sampling Quota:  layer_type x munition")
    ...
    ax.set_title("Group Leakage Check:  is_crawled overlay (X-Z)")
    ...
    if split_manifest_path.exists():
        ...
        for split, idx in split_idx.items():
            ...
            for mid, c in sub_df["munition_id"].value_counts().items():
                rows.append({"split": split.replace("_idx", ""),
                             "munition": MUN_NAMES[mid],
                             "count": int(c)})
        plot_df = pd.DataFrame(rows)
        sns.barplot(x="munition", y="count", hue="split",
                    data=plot_df, palette="Set1", ax=ax)
        ax.set_title("Split Balance:  munition per split")
    else:
        ax.text(0.5, 0.5, "split_manifest.json not found",
                ha="center", va="center")
        ax.set_axis_off()
    ...
    if split_idx is not None:
        ...
        sns.barplot(x="label", y="rate(%)", hue="split",
                    data=plot_df, palette="Set1", ax=ax)
        ax.set_title("Split Balance:  positive-class rate per label")
    else:
        ax.text(0.5, 0.5, "split_manifest.json not found",
                ha="center", va="center")
        ax.set_axis_off()
...
def run_diagnostic_visualization(parquet_path: str = "output/damage_dataset.parquet",
    ...
    ess = (w.sum() ** 2) / (w ** 2).sum()
    print("\n" + "=" * 60)
    print(" 仿真数据集检验诊断简报 (Validation Summary) ")
    print("=" * 60)
    print(f"  样本总数 (Total Samples)         : {len(df):,}")
    print(f"  特征维度 (Feature Dim)           : {len(FEATURE_COLUMNS)}")
    print(f"  速度值域 (Velocity Range)        : "
          f"{df['norm_velocity'].min():.1f} ~ {df['norm_velocity'].max():.1f} m/s")
    print(f"  半径值域 (Radial Range)          : "
          f"{df['radial_dist'].min():.1f} ~ {df['radial_dist'].max():.1f} cm")
    print(f"  气动攻角 (AoA Range)             : "
          f"{df['aerodynamic_aoa'].min():.1f} ~ {df['aerodynamic_aoa'].max():.1f} deg")
    print(f"  IPW权重峰值 (Max Loss Weight)    : {w.max():.2f}")
    print(f"  IPW有效样本 (ESS / N)            : "
          f"{ess:,.0f} / {len(w):,}  ({ess/len(w)*100:.1f}%)")
    print(f"  穿透次数均值 (Avg Penetrations)  : {df['total_penetrations'].mean():.2f}")
    print(f"  灾难性杀伤率 (K-Kill Ratio)      : "
