...
REPORT_SCHEMA = "stage0_nn_a36_validation_promotion_v1"
CANDIDATE_ID = "A36_component_rule_low_fpr_continuation"
BASELINE_ID = "A35_independent_component_tree_fusion"


def evaluate_a36_promotion(
        baseline: dict, candidate: dict) -> dict:
    """Apply the frozen broad-gain rule to the A36 continuation model."""
...
def _assert_test_split_is_sealed(candidate_run_dir: Path) -> None:
    ...
    if present:
        raise RuntimeError(
            "A36 test split is no longer sealed; promotion audit refused: "
            + ", ".join(present)
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Pre-registered, test-blind A36 validation promotion gate. "
            "The candidate is compared with A35 using the already frozen "
            "broad-gain criteria."
        )
    )
