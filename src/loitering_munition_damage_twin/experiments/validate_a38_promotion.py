...
REPORT_SCHEMA = "stage0_nn_a38_validation_promotion_v1"
CANDIDATE_ID = "A38_component_proxy_feature_extension"
BASELINE_ID = "A35_independent_component_tree_fusion"


def evaluate_a38_promotion(
        baseline: dict, candidate: dict) -> dict:
    """Apply the frozen broad-gain rule to the A38 validation candidate."""
...
def _assert_test_split_is_sealed(candidate_run_dir: Path) -> None:
    ...
    if present:
        raise RuntimeError(
            "A38 test split is no longer sealed; promotion audit refused: "
            + ", ".join(present)
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Pre-registered, test-blind A38 validation promotion gate. "
            "The component-proxy feature extension is compared with A35 "
            "using the frozen broad-gain criteria."
        )
    )
