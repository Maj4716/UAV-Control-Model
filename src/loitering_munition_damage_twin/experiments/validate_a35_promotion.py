...
REPORT_SCHEMA = "stage0_nn_a35_validation_promotion_v1"


def evaluate_a35_promotion(
        baseline: dict, candidate: dict) -> dict:
    """Apply the frozen A34-era broad-gain rule to the A35 fusion model."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Test-blind A35 validation promotion gate. "
            "It reuses the already frozen A34 broad-gain criteria."
        )
    )
    ...
    if present:
        raise RuntimeError(
            "A35 test split is no longer sealed; "
            "promotion audit refused: "
            + ", ".join(present)
        )

    result = evaluate_a35_promotion(
